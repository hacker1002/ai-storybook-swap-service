...
class AppDbAdapter(Protocol):
    ...
    # get_art_style REMOVED 260812 rev 2 — App DB does not clone `art_styles` and
    # `books.artstyle_id` is dropped; bundle now hardcodes `artStyle: null`. Do NOT
    # restore from an older README (spec 01 + app-db-clone-tables §Hệ quả swap-service).
    async def list_humans(self, book_id: UUID) -> list[dict]: ...
    async def list_voices(self, book_id: UUID) -> list[dict]: ...

    # --- actors (spec 10 — casting resolve phía App) ---
    # Full rows for a snapshot, no pipeline-completeness filter (FE reads batch state
    # to disable presets). Snapshot absent => empty list, mirrors list_remixes.
